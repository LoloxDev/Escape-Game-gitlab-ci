...
class Pricing
{
    ...
    public function calculatePrice(int $playerCount, string $date): float
    {
        ...
        $price = $this->applyDiscount($price, $date);
    ...
    /**
     * Applique une réduction de 10% si la réservation est en semaine.
     * @param float $price
     * @param string $date (format: "Y-m-d H:i")
     * @return float
     */
    public function applyDiscount(float $price, string $date): float
    {
        $dateTime = new DateTime($date);
        if ($this->isWeekday($dateTime)) {
            return $price * (1 - self::WEEKDAY_DISCOUNT);
        }
        return $price;
    }
